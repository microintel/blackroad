...
import {
  ...
  buildFundYearlyReturns, findNavForDate,
} from './fund-sync.js';
...
function resetAccountDrilldown() {
  ...
  document.querySelector('.user-sections-grid')?.classList.remove('has-open');
  document.querySelectorAll('.account-quicknav-chip').forEach(c => c.classList.remove('active'));
...
function openAccountSection(targetId) {
  ...
  document.querySelector('.user-sections-grid')?.classList.add('has-open');
  document.querySelectorAll('.account-quicknav-chip').forEach(c => c.classList.toggle('active', c.getAttribute('href') === '#' + targetId));
  const list = document.getElementById('account-options-list');
  const back = document.getElementById('acct-back-btn');
...
/* Desktop left-rail nav — same drill-down (options hidden until picked)
   as the mobile options list above, just via a persistent sidebar
   instead of a list-then-detail swap. Clicking a nav row opens only
   that section; picking another swaps to it directly. */
document.querySelectorAll('.account-quicknav-chip').forEach(chip => {
  chip.addEventListener('click', (e) => {
    e.preventDefault();
    openAccountSection(chip.getAttribute('href').slice(1));
  });
});
...
function openAllocationForm(dateStr) {
  ...
  // Make sure the Allocation Tracking section itself is the one open —
  // both on mobile (drill-down list) and desktop (nothing shown until
  // picked from the left nav), regardless of whatever was open before.
  openAccountSection('sec-sip-allocations');
...
/* Re-export for the plain inline <script> in index.html (data-goto="stepup"
   handler) — that script runs outside this module's scope. */
window.openAccountSection = openAccountSection;

/* ══════════════════════════════════════════════════════
   Allocation form — auto-fetch NAV for the actual allocation date
   ------------------------------------------------------
   When a fund is linked (see renderFundLinkStatus above), picking the
   "Allocation Date (actual)" looks that day's real NAV up from mfapi.in
   (via the same cached fund history the Fund Performance chart uses)
   and fills in NAV + units for you. Both fields stay plain, editable
   text inputs the whole time — nothing here stops typing a NAV/units
   value manually, before or after the auto-fill, and if no fund is
   linked (or the lookup fails) it just leaves manual entry as-is. */
async function autoFillAllocationNav(dateIso) {
  const statusEl   = document.getElementById('alloc-nav-fetch-status');
  const navInput   = document.getElementById('alloc-nav');
  const unitsInput = document.getElementById('alloc-units');
  if (!dateIso || !navInput) return;

  const lf = settings && settings.linkedFund;
  if (!lf || !lf.schemeCode) {
    if (statusEl) { statusEl.textContent = 'No fund linked — enter NAV manually.'; statusEl.className = 'alloc-fetch-status'; }
    return;
  }

  if (statusEl) {
    statusEl.innerHTML = '<i class="bi bi-arrow-repeat spin"></i> Fetching NAV…';
    statusEl.className = 'alloc-fetch-status';
  }

  try {
    if (fundHistCache.schemeCode !== lf.schemeCode) {
    ...
    const hit = findNavForDate(fundHistCache.data.series, dateIso);
    if (!hit) {
      if (statusEl) { statusEl.textContent = 'No NAV on record for that date yet — enter manually.'; statusEl.className = 'alloc-fetch-status alloc-fetch-warn'; }
      return;
    }

    navInput.value = hit.nav;
    if (allocFormTargetDate) {
      const amount = amountForDate(settings.sipSchedule, allocFormTargetDate);
      if (amount) unitsInput.value = +(amount / hit.nav).toFixed(4);
    }

    if (statusEl) {
      statusEl.innerHTML = hit.exact
        ? `<i class="bi bi-check-circle-fill"></i> NAV fetched for ${hit.actualDate} — ₹${hit.nav}`
        : `<i class="bi bi-info-circle"></i> No trading on ${dateIso} — using ${hit.actualDate}'s NAV (₹${hit.nav})`;
      statusEl.className = 'alloc-fetch-status ' + (hit.exact ? 'alloc-fetch-ok' : 'alloc-fetch-warn');
    }
  } catch (err) {
    console.error(err);
    if (statusEl) { statusEl.textContent = "Couldn't fetch NAV right now — enter manually."; statusEl.className = 'alloc-fetch-status alloc-fetch-err'; }
  }
}

document.getElementById('alloc-allocation-date')?.addEventListener('change', (e) => {
  autoFillAllocationNav(e.target.value);
});
